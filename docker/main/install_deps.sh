...
if [[ "${TARGETARCH}" == "arm64" ]]; then
    ...
    #ArmNN
    wget https://github.com/ARM-software/armnn/releases/download/v23.08/ArmNN-linux-aarch64.tar.gz .
    mkdir -p /rootfs/usr/lib/ArmNN-linux-aarch64
    tar xfvz ./ArmNN-linux-aarch64.tar.gz -C /rootfs/usr/lib/ArmNN-linux-aarch64
    rm ./ArmNN-linux-aarch64.tar.gz
    ./install_deps_armnn.sh
fi
